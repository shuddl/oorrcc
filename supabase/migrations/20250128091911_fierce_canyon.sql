...
CREATE TABLE requirement_sessions (
  ...
  -- Add state validation constraint
  CONSTRAINT requirement_sessions_state_valid CHECK (
    (state ? 'categories' AND jsonb_typeof(state->'categories') = 'array') AND
    (state ? 'responses' AND jsonb_typeof(state->'responses') = 'array')
  )
);
